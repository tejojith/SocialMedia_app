...
def get_post(id: int, responce: Response,db: Session = Depends(database.get_db), current_user: int = Depends(oauth2.get_current_user)):
#
    ...
    post = db.query(model.Post, func.count(model.Vote.post_id).label("votes")).join(model.Vote, model.Post.id == model.Vote.user_id, isouter=True).group_by(model.Post.id).filter(model.Post.id == id).first()
